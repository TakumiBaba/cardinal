...
JST['userpage/detailProfile'] = _.template(
  ...
  <tr>
    <td class='left-key'>結婚歴</td><td class='left-value'><%= martialHistoryArray[martialHistory] %></td>
    <td class='right-key'>子供の有無</td><td class='right-value'><%= childrenArray[hasChild] %></td>
  </tr>
  <% wantMarriageArray = ['---','すぐにでも','2〜3年のうちに','お相手に合わせる','特に決めてない'] %>
  <% wantChildArray = ['---','結婚したら欲しい','お相手と相談したい','いなくても構わない','欲しくない','特に決めてない'] %>
  <tr>
    <td class='left-key'>結婚希望時期</td><td class='left-value'><%= wantMarriageArray[wantMarriage] %></td>
    <td class='right-key'>子どもの希望</td><td class='right-value'><%= wantChildArray[wantChild] %></td>
  </tr>
  <% addressArray = ['---','北海道','青森県','岩手県','宮城県','秋田県','山形県','福島県','茨城県','栃木県','群馬県','埼玉県','千葉県','東京都','神奈川県','新潟県','富山県','石川県','福井県','山梨県','長野県','岐阜県','静岡県','愛知県','三重県',,'滋賀県','京都府','大阪府','兵庫県','奈良県','和歌山県','鳥取県','島根県','岡山県','広島県','山口県','徳島県','香川県','愛媛県','高知県','福岡県','佐賀県','長崎県','熊本県','大分県','宮崎県','鹿児島県','沖縄県'] %>
  ...
  <tr>
    <td class='left-key'>居住地</td><td class='left-value'><%= addressArray[address] %></td>
    <td class='right-key'>出身地</td><td class='right-value'><%= hometownArray[hometown] %></td>
  </tr>
  ...
  <tr>
    <td class='left-key'>ご職業</td><td class='left-value'><%= jobArray[job] %></td>
    <td class='right-key'>年収</td><td class='right-value'><%= income %>万円</td>
  </tr>
  <% educationArray = ['---','中学卒','高校卒','短大卒','大卒','大学院卒','その他'] %>
  <% bloodTypeArray = ['---','A','B','O','AB'] %>
  <tr>
    <td class='left-key'>学歴</td><td class='left-value'><%= educationArray[education] %></td>
    <td class='right-key'>血液型</td><td class='right-value'><%= bloodTypeArray[bloodType] %></td>
  </tr>
  <% shapeArray = ['---','スリム','ふつう','ぽっちゃり','グラマー','ガッチリ','太め'] %>
  <tr>
    <td class='left-key'>身長</td><td class='left-value'><%= height %>cm</td>
    <td class='right-key'>体型</td><td class='right-value'><%= shapeArray[shape] %></td>
  </tr>

  <% drinkingArray = ['---','毎日飲む','週3～4日飲む','週1～2日程度','たまに飲む','全く飲まない'] %>
  <% smokingArray = ['---','よく吸う','たまに吸う','まったく吸わない'] %>
  <tr>
    <td class='left-key'>飲酒習慣</td><td class='left-value'><%= drinkingArray[drinking] %></td>
    <td class='right-key'>喫煙習慣</td><td class='right-value'><%= smokingArray[smoking] %></td>
  </tr>
  """
  )
